...
class HandlerClass:
    def __init__(self, halcomp, widgets, paths):
        ...
        self.onoff_list = ["frame_mode", "frame_tool", "frame_dro", "frame_override", "frame_status"]
...
""" %(os.path.join(paths.IMAGEDIR,'lcnc_swoop.png'))
    ...
    # macro widget
    def btn_g53xyz_toHome_clicked(self):
        if not STATUS.is_auto_mode():
            self.w.main_tab_widget.setCurrentIndex(0)
            self.w.btn_main.setChecked(True)
            jogvel = int(INFO.DEFAULT_LINEAR_JOG_VEL)
            homeX = float(INFO.INI.find("JOINT_0", "HOME"))	
            homeY = float(INFO.INI.find("JOINT_1", "HOME"))	
            homeZ = float(INFO.INI.find("JOINT_2", "HOME"))
            ACTION.CALL_MDI("G53 G1 Z0 F{}".format(jogvel)) #move Z to G53 ZERO - safe retract
            command = "G53 G1 X{:4.3f} Y{:4.3f} F{}".format(homeX, homeY, jogvel)            
            ACTION.CALL_MDI(command)                        #move XY to G53 HOME
            command = "G53 G1 Z{:4.3f} F{}".format(homeZ,jogvel)
            ACTION.CALL_MDI(command)                        #move Z to G53 HOME
        else:
            self.add_status("Cannot send MDI command while in AUTO mode")

    def btn_g53z_toZero_clicked(self):
        if not STATUS.is_auto_mode():
            self.w.main_tab_widget.setCurrentIndex(0)
            self.w.btn_main.setChecked(True)
            jogvel = int(INFO.DEFAULT_LINEAR_JOG_VEL)
            ACTION.CALL_MDI("G53 G1 Z0 F{}".format(jogvel))
        else:
            self.add_status("Cannot send MDI command while in AUTO mode")

    def btn_g54xy_toZero_clicked(self):        
        if not STATUS.is_auto_mode():
            self.w.main_tab_widget.setCurrentIndex(0)
            self.w.btn_main.setChecked(True)
            jogvel = int(INFO.DEFAULT_LINEAR_JOG_VEL)
            ACTION.CALL_MDI("G53 G1 Z0 F{}".format(jogvel)) #move Z to G53 ZERO - safe retract
            ACTION.CALL_MDI("G54 G1 X0 Y0 F{}".format(jogvel)) #move XY to G54 ZERO
        else:
            self.add_status("Cannot send MDI command while in AUTO mode")

    def btn_g54z_toZero_clicked(self):        
        if not STATUS.is_auto_mode():
            self.w.main_tab_widget.setCurrentIndex(0)
            self.w.btn_main.setChecked(True)
            jogvel = int(INFO.DEFAULT_LINEAR_JOG_VEL)
            ACTION.CALL_MDI("G54 G1 Z0 F{}".format(jogvel)) #move Z to G54 ZERO
        else:
            self.add_status("Cannot send MDI command while in AUTO mode")

    def btn_changePosition_clicked(self):
        if not STATUS.is_auto_mode():
            self.w.main_tab_widget.setCurrentIndex(0)
            self.w.btn_main.setChecked(True)
            jogvel = int(INFO.DEFAULT_LINEAR_JOG_VEL)			
            homeX = float(INFO.INI.find("CHANGE_POSITION", "X"))	
            homeY = float(INFO.INI.find("CHANGE_POSITION", "Y"))	
            homeZ = float(INFO.INI.find("CHANGE_POSITION", "Z"))
            command = "G53 G1 Z{:4.3f} F{}".format(homeZ,jogvel)		
    ...
    def btn_rearPosition_clicked(self):
        if not STATUS.is_auto_mode():
            self.w.main_tab_widget.setCurrentIndex(0)
            self.w.btn_main.setChecked(True)
            jogvel = int(INFO.DEFAULT_LINEAR_JOG_VEL)			
            homeX = float(INFO.INI.find("JOINT_0", "MIN_LIMIT"))+0.1
            homeY = float(INFO.INI.find("JOINT_1", "MAX_LIMIT"))-0.1
            homeZ = float(INFO.INI.find("JOINT_2", "MAX_LIMIT"))-0.1
            command = "G53 G1 Z{:4.3f} F{}".format(homeZ,jogvel)
            ACTION.CALL_MDI(command)
            command = "G53 G1 X{:4.3f} Y{:4.3f} F{}".format(homeX, homeY, jogvel)
            ACTION.CALL_MDI(command)
    ...
    #stackedWidget_2
    def actbtn_man_clicked(self):
        self.w.stackedWidget.setCurrentIndex(4) 
    def actbtn_auto_clicked(self):
        return
        #self.w.stackedWidget.setCurrentIndex(0)
    def actbtn_MDI_clicked(self):
        return
        #self.w.stackedWidget.setCurrentIndex(0)
